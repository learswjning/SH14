...
usv2 = Usv(init_position=[2000, 4635], init_theta=0, id='2')

# 初始化一个目标
target = Target(init_position=[6000, 4635], init_theta=0, id='1')
...
while True:
    ...
    # UAV 前进 + 探测目标
    uav1.update(100, 0, uavs=[uav1,uav2], usvs=[usv1,usv2], targets=[target])
    uav2.update(0, 0, uavs=[uav1,uav2], usvs=[usv1,usv2], targets=[target])

    # USV 保持静止（但能展示圆形探测范围）
    usv1.update(0, 0.0, uavs=[uav1,uav2], usvs=[usv1,usv2], targets=[target])
